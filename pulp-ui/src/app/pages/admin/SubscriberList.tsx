...
import { selectFounderFriendlyName, selectIdentity, selectPublication, selectSubscriberList } from 'store/app-state';
...
export function SubscriberList() {
  ...
  const subscriber_list = useSelector(selectSubscriberList);
        ...
        {!publication?.loading && publication?.entity?.articles?.length && (
          <>
            ...
            <Row className="text-left mb-5">
              <List>
                {Object.entries(subscriber_list?.entity?.subscribers || {}).map(([s, a], i) => (
                  <Feature key={i}>
                    {/* <CSSIcon className="feature-icon" /> */}
                    <Content>
                      <div className="lead">{a.sender}</div>
                      <div className="lead">{a.flowRate}</div>
                    </Content>
                  </Feature>
                ))}
              </List>
